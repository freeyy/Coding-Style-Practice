...
def leet_layer_set_weights(n_layer):
    wb = []
    w = np.zeros((INPUT_VOCAB_SIZE, INPUT_VOCAB_SIZE), dtype=np.float32)
    b = np.zeros((INPUT_VOCAB_SIZE), dtype=np.float32)
    # map lower case letters to leet
    for c in string.ascii_lowercase:
        i = char_indices[c]
        il = char_indices[c] - char_indices['a']
        w[i, il] = 1
    # Map capitals to leet
    for c in string.ascii_uppercase:
        i = char_indices[c]
        il = char_indices[c.lower()] - char_indices['a']
        w[i, il] = 1
    # Map all non-letters to 30
    sp_idx = 30
...
model = build_model()
model.summary()
leet_layer_set_weights(model.layers[0])
